...
class AdaptiveReflectionGoalTests(unittest.TestCase):
  def test_goal_stages_meta_model_and_bounded_system_evidence(self):
    ...
    self.assertIn("only about listed exceptions", goal)
  ...
  def test_goal_names_activity_and_question_engagement_evidence(self):
    ...
    self.assertIn("Engagement is", goal)
    self.assertIn("valid non-empty question", goal)
    self.assertIn("that exact", goal)
    self.assertIn("A missing/empty carrier asked", goal)
    self.assertIn("an older answer proves nothing", goal)
